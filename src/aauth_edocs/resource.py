...
beyond dataflow matching belong in the view (see flask.g.aauth).
"""
...
def install_resource(app: Flask, config: ResourceConfig) -> None:
    ...
    def authorization_endpoint():
        # §6.1: signed POST with dataflow; agent from signature
        try:
            ...
            if dataflow is None:
                raise AAuthError(INVALID_REQUEST, 400, "dataflow is required")
            try:
                dataflow = validate_dataflow(dataflow)
            except ValueError as error:
                raise AAuthError(INVALID_REQUEST, 400, str(error)) from error
            return {"resource_token": _mint_resource_token(config, verified, dataflow=dataflow)}
...
def require_auth_token(config: ResourceConfig, dataflow: dict):
    ...
    dataflow, then run the view with g.aauth set. Agent-token-signed
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if typ == AGENT_TYP:  # §6.6: challenge with a fresh resource token
                    token = _mint_resource_token(config, verified, dataflow=dataflow)
                    return (
